...
import { useParams, useNavigate } from "react-router-dom"; // Import de useNavigate pour gérer les redirections
...
export default function Lodging() {
    const { id } = useParams();
    const navigate = useNavigate();  // Initialisation de la fonction navigate pour permettre les redirections
    ...
    useEffect(() => {
        ...
        .then(function(json){
            ...
            if(match) {
                ...
                navigate("/404");  // Si le logement avec l'ID spécifié n'est pas trouvé, on redirige l'utilisateur vers la page d'erreur 404
            }
        })
        .catch(function(err){
            console.log(err);
        });
    }, [id, navigate]); // L'ajout de navigate comme dépendance de useEffect pour s'assurer que cela fonctionne correctement
